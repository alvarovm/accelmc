...
double total_e ( double * rx, double * ry, double * rz, int N, double L,
   ...
   const double hL=L/2.0;
   double e=0;

   *vir=0.0;
   double vv=0;
#pragma acc parallel  loop reduction(+:e,vv) copyin(ry[:N],rx[:N],rz[:N])
   for (int i=0;i<(N-1);i++) {
     for (int j=i+1;j<N;j++) {
	double dx  = (rx[i]-rx[j]);
	double dy  = (ry[i]-ry[j]);
	...
	const double r2 = dx*dx + dy*dy + dz*dz;
	  if (r2<rc2) {
	    const double r6i   = 1.0/(r2*r2*r2);
	    e   += 4*(r6i*r6i - r6i) - (shift?ecut:0.0);
	    vv += 48*(r6i*r6i-0.5*r6i);
    	}
     }
   }
   *vir=vv;
...
int main ( int argc, char * argv[] ) {
  ...
  double rc = 2.5, rc2 = rc*rc, vir, p_sum, rho_sum, pcor;
